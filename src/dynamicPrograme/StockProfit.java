...
/**
 * @Author:毛翔宇
 * @Date 2019-08-11 23:52
 */
public class StockProfit {

    /*
    top-down 采用制表+递归搜索的方法，但运行超时
    此方法利用递归的思维进行穷举
     */
    private int[][] res;
    public int maxProfit(int k, int[] prices){
        res = new int[prices.length][k];
        entry(0,k,prices);
        int max = res[0][k-1];
        for (int i = 1;i<res.length;i++)
            max = Math.max(res[i][k-1],max);
        return max;
    }
    public int entry(int day, int transaction, int[] prices){
        if (day == prices.length || transaction == 0)
            return 0;
        for (int j = day;j<prices.length;j++) {
            if (res[j][transaction-1] == 0)
                profit(j, transaction, prices);
            res[day][transaction-1] = Math.max(res[day][transaction-1],res[j][transaction-1]);
        }
        return res[day][transaction-1];
    }
    public void profit(int begin, int k, int[] prices){
        for (int index = begin+1;index<prices.length;index++){
            if (prices[index] >= prices[begin])
                res[begin][k - 1] = Math.max(res[begin][k - 1], prices[index] - prices[begin] + entry(index + 1, k - 1, prices));
        }
    }
    /*
    利用状态分析思想穷举本题
